...
```bash
# 需要 Python ≥3.12 与 Node.js（供无痕验证求解器使用）
uv sync                                   # 安装 Python 依赖（基于 uv.lock）
cd captcha_node && npm install && cd ..   # 无痕验证求解器依赖（Node + jsdom）
cp .env.example .env                      # 按需修改

uv run python main.py serve               # 启动网关 + 后台 UI（默认端口 3000）
```
...
```bash
uv run python main.py serve [--port 3000]           # 启动服务
uv run python main.py login zai [--no-browser]      # OAuth 登录 Z.AI 并自动入池
uv run python main.py add-account zai <name> <jwt|key>    # 添加轮询账号
uv run python main.py accounts [zai|bigmodel]             # 查看账号列表
uv run python main.py remove-account <provider> <id|name> # 删除账号
uv run python main.py quota                               # 查看各账号实时额度
uv run python main.py status                              # 查看配置概览
uv run python main.py set-admin-key <key>                 # 设置后台密码
uv run python main.py export [file] / import <file>       # 导出 / 导入账号
```
